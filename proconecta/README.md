...
- **Ordem de Serviço** (antes "Aprovação de visitas"): mostra todos os cards de O.S. do mês selecionado (com a mesma navegação ‹ mês › do calendário), não só as pendentes de aprovação. Cada card já indica pelo tipo de ação disponível em que ponto está: sem relatório ainda (aguardando o técnico), relatório enviado (Abrir relatório / Aprovar / Reprovar), ou já aprovado (Abrir relatório / Reabrir / Excluir). Solicitações de reabertura enviadas pelo técnico aparecem destacadas no topo, independente do mês. Os cards (aqui e no calendário da Agenda geral) ficam lado a lado num grid compacto (4 a 5 por linha em telas largas, menos em telas menores), ordenados por data; ao abrir um relatório, o conteúdo completo aparece num painel abaixo do grid em vez de espremer dentro do card estreito.
